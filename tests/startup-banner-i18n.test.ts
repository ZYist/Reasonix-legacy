...
describe("startup.codeRooted", () => {
  it("renders the EN banner with comma + space between rootDir and session", () => {
    ...
    expect(out).toBe(
      '\u25b8 reasonix-legacy code: rooted at /project, session "abc" \u00b7 5 native tool(s) \u00b7 semantic_search on',
    );
  });
  ...
  it("omits the semantic suffix when no semantic engine is on", () => {
    ...
    expect(out).toBe(
      '\u25b8 reasonix-legacy code: rooted at /project, session "(ephemeral)" \u00b7 0 native tool(s)',
    );
  });
  ...
  it("renders the zh-CN banner with the CJK comma between rootDir and session", () => {
    ...
    expect(out).toBe(
      '▸ reasonix-legacy code：根目录 /项目，会话 "abc" · 5 个原生工具 · 语义搜索已开启',
    );
  });
});
