...
describe("planUpdate", () => {
  ...
  it("emits npm install -g for npm-installed binaries", () => {
    ...
    expect(plan.command).toEqual(["npm", "install", "-g", "reasonix-legacy@latest"]);
  });
  ...
  it("emits bun add -g for bun-installed binaries", () => {
    ...
    expect(plan.action).toBe("run-install");
    expect(plan.command).toEqual(["bun", "add", "-g", "reasonix-legacy"]);
  ...
  it("emits pnpm add -g for pnpm-installed binaries", () => {
    ...
    expect(plan.action).toBe("run-install");
    expect(plan.command).toEqual(["pnpm", "add", "-g", "reasonix-legacy@latest"]);
  });
  ...
  it("emits yarn global add for yarn-installed binaries", () => {
    ...
    expect(plan.action).toBe("run-install");
    expect(plan.command).toEqual(["yarn", "global", "add", "reasonix-legacy@latest"]);
  });
  ...
  it("returns manual-hint when source cannot be detected — no silent npm fallback", () => {
    ...
    expect(plan.command).toBeUndefined();
    expect(plan.message).toContain("npm install -g reasonix-legacy@latest");
    expect(plan.message).toContain("bun add -g reasonix-legacy");
    expect(plan.message).toContain("pnpm add -g reasonix-legacy@latest");
    expect(plan.message).toContain("yarn global add reasonix-legacy@latest");
  });
});
...
describe("updateCommand", () => {
  ...
  it("spawns npm install -g when npm-installed and behind latest", async () => {
    ...
    expect(h.spawnCalls).toEqual([["npm", "install", "-g", "reasonix-legacy@latest"]]);
    expect(h.exitCode).toBeUndefined();
  });
  ...
  it("spawns bun add -g when bun-installed", async () => {
    ...
    expect(h.spawnCalls).toEqual([["bun", "add", "-g", "reasonix-legacy"]]);
  });
  ...
  it("refuses with manual hint and exits 1 when source cannot be detected", async () => {
    ...
    expect(joined).toContain("npm install -g reasonix-legacy@latest");
    expect(joined).toContain("bun add -g reasonix-legacy");
    expect(h.spawnCalls).toHaveLength(0);
