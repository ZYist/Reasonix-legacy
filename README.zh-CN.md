...
<p align="center">
  ...
  <strong>简体中文</strong>
  &nbsp;·&nbsp;
  <a href="./REASONIX.md">官方网站</a>
  &nbsp;·&nbsp;
  <a href="./REASONIX.md?lang=zh">配置指南</a>
  &nbsp;·&nbsp;
...
其他子命令（`replay` · `diff` · `events` · `stats` · `index` · `mcp` · `prune-sessions`）在 `reasonix --help` 和 [CLI 参考](./REASONIX.md)。

### QQ 通道

Reasonix 通过独立的 headless CLI 命令提供 QQ、Telegram 和微信通道，共用同一 agent core，不依赖桌面端或 Web host。

- CLI：运行 `reasonix qq`（Telegram 与微信分别使用 `reasonix telegram`、`reasonix weixin`）

连接成功后，QQ 消息可以进入当前会话，助手回复会回到 QQ，后续确认和跟进交互也可以继续在 QQ 上完成。

完整配置与 CLI 排障说明见：[QQ 连接指南](./docs/qq-connect.zh-CN.md)。
...
> 📘 **[配置指南](./REASONIX.md?lang=zh)** · [English](./REASONIX.md)

| 主题 | 速读 |
|---|---|
| [MCP 服务器](./REASONIX.md?lang=zh#mcp) | stdio · SSE · Streamable HTTP。`config.json` 和 `--mcp` 共用同一种 spec 格式。 |
| [Skills](./REASONIX.md?lang=zh#skills) | 模型可以调用的 markdown 剧本。`inline` 或 `subagent` 两种模式。 |
| [Memory](./REASONIX.md?lang=zh#memory) | 用户私有的知识，钉进前缀。`user` / `feedback` / `project` / `reference` 四类。 |
| [Hooks](./REASONIX.md?lang=zh#hooks) | 生命周期事件触发的 shell 命令。`PreToolUse`（拦截）· `PostToolUse` · `UserPromptSubmit` · `Stop`。 |
| [权限](./REASONIX.md?lang=zh#permissions) | 按工作区的 shell 白名单，精确前缀匹配。 |
| [Web 搜索](./REASONIX.md?lang=zh#search) | 默认 Bing；可用 `/search-engine` 切到百度 AI Search、自托管 SearXNG、Metaso、Tavily、Perplexity、Exa、Brave 或 Ollama。 |
| [语义索引](./REASONIX.md?lang=zh#index) | `reasonix index` —— 本地 Ollama，或任何 OpenAI 兼容的 embedding 接口。 |
...
- [**官方网站**](./REASONIX.md) —— 入门、Dashboard 设计稿、TUI 设计稿
