    ...
    fn: async (args: { task?: unknown }, ctx) => {
      if (!subagentRunner) {
        return JSON.stringify({
          error: `${spec.toolName}: no subagent runner is configured for this session — run inside \`reasonix-legacy code\`, or pass \`subagentRunner\` to \`registerSkillTools\`.`,
        });
      }
  ...
  registry.register({
    ...
    fn: async (args: { name?: unknown; arguments?: unknown }, ctx) => {
      ...
      if (skill.runAs === "subagent") {
        if (!subagentRunner) {
          return JSON.stringify({
            error: `run_skill: skill ${JSON.stringify(name)} is marked runAs=subagent but no subagent runner is configured for this session. Skill authors who need isolation should run inside reasonix-legacy code (or a library setup that passes subagentRunner to registerSkillTools).`,
          });
        }
        if (!rawArgs) {
          return JSON.stringify({
  ...
  registry.register({
    ...
    fn: async (args: {
      ...
      if (scope === "project" && !hasProjectScope) {
        return JSON.stringify({
          error:
            "install_skill: scope='project' requires a workspace — run from `reasonix-legacy code`, or use scope='global'",
        });
      }
