    ...
    parameters: {
      ...
      properties: {
        ...
        scope: {
          ...
          enum: ["global", "project"],
          description:
            "global = across all projects; project = current sandbox only (needs `reasonix-legacy code`).",
        },
        name: {
          type: "string",
          description:
    ...
    fn: async (args: {
      ...
      if (args.scope === "project" && !hasProject) {
        return JSON.stringify({
          error:
            "scope='project' is unavailable in this session (no sandbox root). Retry with scope='global', or ask the user to switch to `reasonix-legacy code` for project-scoped memory.",
        });
      }
      try {
