...
export interface AppProps {
   ...
   * apply them to disk under `rootDir`. Set by `reasonix-legacy code`. The
...
function AppInner({
  ...
  const startWalkthrough = useCallback((): string => {
    if (!codeMode) {
      return "/walk is only available inside `reasonix-legacy code`.";
    }
    ...
    <>
      <TickerProvider disabled={tickerSuspended}>
        <InflightProvider inflight={loop.inflight}>
          <Box flexDirection="row" backgroundColor={SURFACE.bg} height={viewportRows}>
            <Box
              flexDirection="column"
              flexGrow={planPanelOpen ? 0 : 1}
              width={planPanelOpen ? "35%" : undefined}
            >
              <Box flexDirection="column" flexGrow={1} flexShrink={1} height="100%">
                    ...
                    onChoose={(outcome) => {
                      if (outcome.kind === "open") {
                        ...
                        if (onSwitchSession) {
                          ...
                          log.pushInfo(
                            `to switch to "${outcome.name}", quit and run: reasonix-legacy chat --session ${outcome.name}`,
                          );
                        }
                        return;
                      }
                      if (outcome.kind === "new") {
                        ...
                        if (onSwitchSession) {
                          ...
                          log.pushInfo(
                            "to start a fresh session, quit and run: reasonix-legacy chat (no --session flag)",
                          );
                        }
                        return;
                      }
