...
export interface ChatOptions {
   ...
   * `reasonix-legacy code` to register native filesystem tools in place of
   ...
   * Set by `reasonix-legacy code`; plain `reasonix-legacy chat` leaves this off.
   */
  codeMode?: {
    rootDir: string;
...
export async function chatCommand(opts: ChatOptions): Promise<void> {
  ...
  // Seed registry from the caller (e.g. reasonix-legacy code's native
  ...
  // the seedTools path from `reasonix-legacy code` (which registers its own
