...
export interface RunOptions {
  ...
  system: string;
  budgetUsd?: number;
  /** JSONL transcript path — lets `reasonix-legacy replay` / `diff` audit this run. */
  transcript?: string;
...
export async function runCommand(opts: RunOptions): Promise<void> {
  ...
  if (normalizedSpecs.length > 0) {
    ...
    for (const spec of normalizedSpecs) {
      ...
      try {
        ...
        // Non-fatal — skip and continue, same as `reasonix-legacy chat`. A
  ...
  try {
    for await (const ev of loop.step(opts.task)) {
      ...
      if (ev.role === "assistant_final" && ev.stats?.usage) {
        // `reasonix-legacy run` is often used in CI / scripting — we want
        // those turns to show up in `reasonix-legacy stats` too so the
  ...
  if (opts.transcript) {
    ...
    process.stdout.write(`  → npx reasonix-legacy replay ${opts.transcript}\n`);
  }
