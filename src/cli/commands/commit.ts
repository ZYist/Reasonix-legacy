...
function dieIfNotGitRepo(): void {
  ...
  if (r.status !== 0) {
    process.stderr.write("reasonix-legacy commit: not inside a git repository.\n");
    process.exit(1);
  }
}
...
function readDiff(): DiffResult | null {
  ...
  if (staged.status !== 0) {
    process.stderr.write(
      `reasonix-legacy commit: git diff --staged failed: ${staged.stderr.trim()}\n`,
    );
    process.exit(1);
  }
...
function editInExternal(initial: string): string | null {
  ...
  if (!editor) {
    process.stderr.write(
      "reasonix-legacy commit: no $EDITOR / $VISUAL / $GIT_EDITOR set — can't open editor. Pick [a]ccept and `git commit --amend` afterwards.\n",
    );
    return null;
  }
  ...
  if (result.status !== 0) {
    ...
    process.stderr.write(
      `reasonix-legacy commit: editor exited ${result.status} — keeping prior draft.\n`,
    );
    return null;
  }
  let edited: string;
...
function commitWithMessage(message: string): void {
  ...
  child.on("close", (code) => {
    if (code !== 0) {
      process.stderr.write(`reasonix-legacy commit: git commit exited ${code}.\n`);
      process.exit(code ?? 1);
    }
  });
}
...
export async function commitCommand(opts: CommitOptions = {}): Promise<void> {
  ...
  if (!ep.apiKey) {
    process.stderr.write(
      "reasonix-legacy commit: DEEPSEEK_API_KEY not set. Run `reasonix-legacy setup` to save one, or export it.\n",
    );
    process.exit(1);
  }
  ...
  if (!diff) {
    process.stderr.write(
      "reasonix-legacy commit: no staged changes and working tree is clean — nothing to commit.\n",
    );
    process.exit(1);
  }
  if (diff.source === "working-tree") {
    process.stderr.write(
      "reasonix-legacy commit: nothing staged — drafting from working-tree diff. Stage your changes and re-run, or use the draft as a starting point.\n",
    );
  }
  if (diff.truncated) {
    process.stderr.write(
      "reasonix-legacy commit: diff exceeded 80KB; head + tail sent to the model. Large diffs often produce vague drafts — consider committing in smaller chunks.\n",
    );
  }
  ...
  while (true) {
    ...
    try {
      ...
      process.stderr.write(
        `reasonix-legacy commit: model call failed — ${(err as Error).message}\n`,
      );
      process.exit(1);
    }
    if (!message) {
      process.stderr.write("reasonix-legacy commit: model returned an empty draft. Try again.\n");
      process.exit(1);
    }
    printDraft(message);
