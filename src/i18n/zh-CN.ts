...
export const zhCN = {
  ...
  stats: {
    usageHint:
      "运行 `reasonix-legacy chat`、`reasonix-legacy code` 或 `reasonix-legacy run <task>` — 每次对话都会记录",
    usageDetail: "每次对话在日志中追加一行，`reasonix-legacy stats` 会将其汇总统计。",
  },
  run: {
    missingApiKey:
      "未设置 DEEPSEEK_API_KEY 且标准输入不是 TTY（无法交互式输入）。\n" +
      "请设置环境变量，或先运行 `reasonix-legacy chat` 交互一次以保存密钥。\n",
  },
  sessions: {
    emptyHint:
      "暂无已保存的会话 — 运行 `reasonix-legacy chat`（会话会自动保存，除非使用了 --no-session）。",
    listHeader: "保存的会话 (~/.reasonix/sessions/)：",
    inspectHint: "查看：reasonix-legacy sessions <name>",
    resumeHint: "恢复：reasonix-legacy chat --session <name>",
  ...
  ui: {
    welcome: "随时运行 `reasonix-legacy` 开始聊天 — 您的设置将被记住。",
  ...
  slash: {
    ...
    setup: { description: "提醒您退出并运行 `reasonix-legacy setup`" },
    semantic: {
  ...
  errors: {
    ...
    auth401:
      "认证失败（DeepSeek 401）：{inner}。你的 API key 被拒绝。运行 `reasonix-legacy setup` 或 `export DEEPSEEK_API_KEY=sk-...` 修复。在 https://platform.deepseek.com/api_keys 获取 key。",
    balance402:
  ...
  handlers: {
    basic: {
      ...
      helpSessionCustom: "  reasonix-legacy chat --session <name>   使用不同的命名会话",
      helpSessionNone: "  reasonix-legacy chat --no-session       禁用本次运行的持久化",
    ...
    sessions: {
      ...
      persistSetOn:
        "▸ session-persist 已设为 on — 下次 `reasonix-legacy code/chat` 将恢复上次会话。",
      persistSetOff:
    ...
    admin: {
      ...
      updateCurrent: "当前：{version}",
      updateLatestPending: "最新：（尚未解析 — 后台检查进行中或离线）",
      updateRetryHint: "已触发新的注册表获取 — 几秒后重试 `/update`，",
      updateRetryHint2: "或在另一个终端运行 `reasonix-legacy update` 强制同步执行。",
      updateLatest: "最新：{version}",
      updateUpToDate: "您已是最新版本。无需操作。",
      updateNpxHint: "您正在通过 npx 运行 — 下次 `npx reasonix-legacy ...` 启动时将自动获取。",
      updateNpxForce: "要强制刷新：`npm cache clean --force`。",
      updateUpgradeHint: "要升级，请退出此会话并运行：",
      updateUpgradeCmd1: "  reasonix-legacy update           （交互式，支持 --dry-run 预览）",
    ...
    edits: {
      undoCodeOnly: "/undo 仅在 `reasonix-legacy code` 中可用 — 聊天模式不应用编辑。",
      historyCodeOnly: "/history 仅在 `reasonix-legacy code` 中可用。",
      showCodeOnly: "/show 仅在 `reasonix-legacy code` 中可用。",
      applyCodeOnly: "/apply 仅在 `reasonix-legacy code` 中可用（此处无内容可应用）。",
      discardCodeOnly: "/discard 仅在 `reasonix-legacy code` 中可用。",
      planCodeOnly: "/plan 仅在 `reasonix-legacy code` 中可用 — 聊天模式不限制工具写入。",
      planOn:
      ...
      modeCodeOnly: "/mode 仅在 `reasonix-legacy code` 中可用。",
      ...
      commitCodeOnly: "/commit 仅在 `reasonix-legacy code` 中可用（需要有根的 git 仓库）。",
      commitUsage: '用法：/commit "提交消息"  — 在 {root} 中运行 `git add -A && git commit -m "…"`',
      walkCodeOnly: "/walk 仅在 `reasonix-legacy code` 中可用。",
      checkpointCodeOnly: "/checkpoint 仅在 `reasonix-legacy code` 中可用 — 聊天模式不应用编辑。",
      checkpointNone:
      ...
      restoreCodeOnly: "/restore 仅在 `reasonix-legacy code` 中可用。",
      ...
      cwdCodeOnly: "/cwd 仅在 `reasonix-legacy code` 中可用。",
      cwdUsage:
    ...
    permissions: {
      mutateCodeOnly:
        "/permissions add / remove / clear 仅在 `reasonix-legacy code` 中可用 — 它们编辑项目范围的允许列表（`~/.reasonix/config.json` projects[<root>].shellAllowed）。",
      addUsage:
    ...
    plans: {
      noSession:
        "未附加会话 — `/plans` 是按会话的。在项目中运行 `reasonix-legacy code` 以获取会话。",
      ...
      replayNoSession:
        "未附加会话 — `/replay` 是按会话的。在项目中运行 `reasonix-legacy code` 以获取会话。",
      replayNoArchives:
    ...
    jobs: {
      codeOnly: "/jobs 仅在 `reasonix-legacy code` 中可用。",
      killCodeOnly: "/kill 仅在 `reasonix-legacy code` 中可用。",
      logsCodeOnly: "/logs 仅在 `reasonix-legacy code` 中可用。",
      empty:
    ...
    mcp: {
      noServers:
        '未附加 MCP 服务器。运行 `reasonix-legacy setup` 选择一些，或使用 --mcp "<spec>" 启动。`reasonix-legacy mcp list` 显示目录。注：模型发起的 shell 命令按次审批（allow once / allow always / deny），设计上没有「全局放行」开关。',
      toolsLabel: "  工具     {count}",
      resourcesHint: "`/resource` 浏览+读取",
      promptsHint: "`/prompt` 浏览+获取",
      awarenessOnly: "聊天模式目前消耗工具；资源+提示在此展示供了解。",
      catalogHint:
        "完整目录：`reasonix-legacy mcp list` · 深度诊断：`reasonix-legacy mcp inspect <spec>`。",
      fallbackServers: "MCP 服务器（{count}）：",
      fallbackTools: "注册表中的工具（{count}）：",
      fallbackChange: "要更改此设置，请退出并运行 `reasonix-legacy setup`。",
      usageDisableEnable:
    ...
    init: {
      codeOnly:
        "/init 仅在代码模式下工作（需要文件系统工具）。\n运行 `reasonix-legacy code [path]` 启动一个以您要初始化的项目为根的会话，\n然后运行 /init。",
    ...
    skill: {
      ...
      listProjectOnly: "  （项目范围仅在 `reasonix-legacy code` 中活跃）",
  ...
  startup: {
    codeRooted:
      '▸ reasonix-legacy code：根目录 {rootDir}，会话 "{session}" · {tools} 个原生工具{semantic}',
    ephemeral: "（临时）",
    semanticOn: " · 语义搜索已开启",
  },
  doctorErrors: {
  ...
  mcpMarketplace: {
    ...
    bridgeReloadFailed:
      "✓ 已安装 {name} — 重启 `reasonix-legacy code` 以桥接（重载失败：{message}）",
    restartBridge: "✓ 已安装 {name} — 重启 `reasonix-legacy code` 以桥接",
    needsEnv: "  ·  需要环境变量：{env}",
    badgeOfficial: "[官方]",
    badgeSmithery: "[三方]",
    badgeLocal: "[本地]",
  ...
  mcpBrowser: {
    title: "◈ MCP 浏览器",
    empty: "没有挂载 MCP 服务器。运行 `reasonix-legacy setup` 选择一些，或使用 --mcp 启动。",
  ...
  mcpLifecycle: {
    ...
    failedSetupHint:
      "→ 运行 `reasonix-legacy setup` 移除此条目，或修复底层问题（缺少 npm 包、网络等）。",
    failedSetupConfigHint: "→ 运行 `reasonix-legacy setup` 从已保存配置中移除损坏的条目。",
  ...
  mcpCli: {
    ...
    moreAvailable: "还有更多",
    allLoaded: "已全部加载",
    morePagesAvailable: "▸ 还有更多页可用 — `reasonix-legacy mcp list --pages <n>` 或 --all",
    installHint: "安装：reasonix-legacy mcp install <name>",
    usageSearch: "用法：reasonix-legacy mcp search <query>",
    usageInstall: "用法：reasonix-legacy mcp install <name>",
    noMatchesFor: '未找到 "{q}" 的匹配项（已检索 {count} 条记录，来源：{source}）',
    matchCount: '在 {source} 中找到 {count} 条 "{q}" 的匹配项（已扫描 {loaded} 条记录）：',
    moreLoaded: "… 还有 {count} 条已加载 — 使用 `reasonix-legacy mcp search <query>` 筛选",
    moreMatches: "… 还有 {count} 条匹配项",
    installed: "已安装：{spec}",
    noServerFound: '在 {source} 中遍历了 {pages} 页后未找到名为 "{target}" 的 MCP 服务器。',
    noServerTryMore: "试试：reasonix-legacy mcp install {target} --max-pages 100",
  ...
  commands: {
    ...
    desktop: {
      retired:
        "`reasonix-legacy desktop` 已下线。若要在终端驱动机器人,请改用 `reasonix-legacy qq`、`reasonix-legacy telegram` 或 `reasonix-legacy weixin`。",
    },
  },
