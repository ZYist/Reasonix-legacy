...
| feature | status | note |
|---|---|---|
| CLI wiring (`reasonix-legacy chat --mcp <cmd>`) | ✅ shipped | see Usage below |
...
reasonix-legacy chat --mcp "node --import tsx examples/mcp-server-demo.ts"

# Official filesystem server:
reasonix-legacy chat --mcp "npx -y @modelcontextprotocol/server-filesystem /tmp/safe-dir"
...
reasonix-legacy run "list files in /tmp/safe-dir" \
