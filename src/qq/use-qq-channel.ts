...
interface UseQQChannelArgs {
  ...
  // Object-injection gate callbacks (D-05/D-06): each field is a plain
  // closure the caller wires — the headless host passes
  // pauseGate.resolve/cancel closures directly; the Ink-TUI caller wraps
  // its ref via `onShellConfirm={(c) => shellConfirmRef.current?.(c)}`.
  // Only the *external* gate-callback contract flipped to object form;
  // internal adapter refs (pendingGateIdRef/interactionRef/etc.) stay refs.
  ...
  const consumePauseReply = useCallback(
    (text: string): boolean => {
      ...
      switch (interaction.kind) {
        case "run_command":
        case "run_background":
          onShellConfirm(parseRunPermissionChoice(text));
          return true;
        case "path_access":
          onPathConfirm(parseRunPermissionChoice(text));
          return true;
        case "plan_proposed": {
        ...
        case "plan_revision":
          onPlanRevision(parseRevisionChoice(text));
          return true;
        case "choice": {
          const payload =
          ...
          if (pickedIndex >= 0 && pickedIndex < options.length) {
            ...
            if (selected) onChoiceResolve({ type: "pick", optionId: selected.id });
            return true;
          }
